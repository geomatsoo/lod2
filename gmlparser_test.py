...
with open('/Users/matthiassoot/Nextcloud/Habil/LOD2-Modelle/Qgis-Test/Aufgabe_1/lgln-opengeodata-lod2.geojson', 'r') as f:
    data = json.load(f)
...
for s in range(1,len(data['features']),50):
    ...
    #for k in range(0,len(data['features'])):
        ...
        for building in root.iter('{http://www.opengis.net/citygml/building/1.0}Building'):
            ...
            hohe = 0
            for re in root[i][0].iter('{http://www.opengis.net/citygml/building/1.0}measuredHeight'):
                if (float(re.text) > hohe):
                    hohe = float(re.text)
            liste.append(hohe)
        ...
        #write to csv
    file = open('/Users/matthiassoot/Desktop/GML/'+str(s)+'.csv', 'w') 
